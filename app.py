...
HF_USERNAME  = "Hamim88"
HF_REPO      = "neuro-scan-ai"
MODEL_FILE   = "brain_classifier_final.pth"
MODEL_URL    = f"https://huggingface.co/Hamim88/neuro-scan-ai/resolve/main/brain_classifier_final.pth"
...
html, body, [class*="css"] {
    ...
    background-color: #080c10; color: #e2e8f0;
}
.stApp { background: #080c10; }
[data-testid="stSidebar"] { background: #0d1117 !important; border-right: 1px solid #1e2a38; }
...
.brain-header h1 {
    ...
    font-size: 1.9rem; color: #f0f9ff;
    letter-spacing: -0.5px; margin-bottom: 6px;
}
.brain-header p { color: #64748b; font-size: 0.92rem; }
...
.metric-box .m-value { font-family: 'Space Mono', monospace; font-size: 1.6rem; font-weight: 700; }
.prob-row { display:flex; align-items:center; gap:12px; padding:8px 0; border-bottom:1px solid #111827; }
.prob-row:last-child { border-bottom: none; }
.prob-label { font-size:0.82rem; color:#94a3b8; width:180px; flex-shrink:0; }
.prob-bar-bg { flex:1; height:8px; background:#1e2a38; border-radius:4px; overflow:hidden; }
.prob-bar-fill { height:100%; border-radius:4px; }
.prob-pct { font-family:'Space Mono',monospace; font-size:0.82rem; color:#64748b; width:52px; text-align:right; flex-shrink:0; }
.sec-title {
    font-family:'Space Mono',monospace; font-size:0.7rem; color:#334155;
...
.upload-zone .uz-title { font-family:'Space Mono',monospace; font-size:1rem; color:#38bdf8; margin-bottom:8px; }
.upload-zone .uz-sub { color:#334155; font-size:0.85rem; }
.model-dot { display:inline-block; width:8px; height:8px; border-radius:50%; margin-right:8px; }
.dot-green { background:#4ade80; box-shadow:0 0 6px #4ade80; }
.dot-red   { background:#f87171; box-shadow:0 0 6px #f87171; }
.sev-badge { display:inline-block; padding:3px 12px; border-radius:4px; font-family:'Space Mono',monospace; font-size:0.75rem; font-weight:700; letter-spacing:1px; }
...
DEFAULT_CLASSES = ['Normal','Glioma Tumor','Meningioma Tumor','Pituitary Tumor','Hemorrhage']
SEVERITY_MAP = {
...
PROB_COLORS = {
    ...
    'Meningioma Tumor':'#fb923c','Pituitary Tumor':'#facc15','Hemorrhage':'#f43f5e',
}


# ── Auto-download model then load ─────────────────────────────────
@st.cache_resource
def load_model():
    try:
        ...
        if not LOCAL_MODEL.exists():
            ...
            with st.spinner(f"⬇️ Downloading model from Hugging Face (~100MB)..."):
        ...
        ck          = torch.load(LOCAL_MODEL, map_location=DEVICE)
...
def make_heatmap(img_pil, model):
    ...
    heat_rgb = plt.colormaps['jet'](heat_up)[:, :, :3]
...
with st.sidebar:
    ...
    if model:
        ...
        st.markdown(
            '<div style="margin-top:8px"><span class="model-dot dot-green"></span>'
            '<span style="color:#4ade80;font-size:0.85rem;font-family:monospace">Model loaded ✓</span></div>',
            unsafe_allow_html=True
        )
        st.markdown("---")
        ...
        for k, v in [
            ...
            st.markdown(
                f'<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #111827">'
                f'<span style="color:#475569;font-size:0.8rem">{k}</span>'
                f'<span style="color:#94a3b8;font-size:0.8rem;font-family:monospace">{v}</span></div>',
                unsafe_allow_html=True
            )
        st.markdown("---")
        ...
        classes = DEFAULT_CLASSES; show_heatmap = True; threshold = 0.5
        st.markdown(
            '<div style="margin-top:8px"><span class="model-dot dot-red"></span>'
            '<span style="color:#f87171;font-size:0.85rem;font-family:monospace">Not loaded</span></div>',
            unsafe_allow_html=True
        )
        if err:
            st.error(f"{err}")
    ...
    st.markdown(
        ...
        'Neuro Scan AI v1.0 · ResNet-50<br>For research use only</div>',
        unsafe_allow_html=True
    )
...
<div class="brain-header">
    ...
    <p>Upload a brain CT scan — detect anomalies &amp; classify disease type with confidence scores.
    &nbsp;·&nbsp; <em>For research use only. Not a medical device.</em></p>
</div>
""", unsafe_allow_html=True)
...
st.markdown(f'<div class="status-pill {pill_cls}">{pill_text}</div>', unsafe_allow_html=True)
...
for col, label, value, color in [
    ...
    col.markdown(
        f'<div class="metric-box"><div class="m-label">{label}</div>'
        f'<div class="m-value" style="color:{color};'
        f'font-size:{"0.9rem" if label=="DIAGNOSIS" else "1.6rem"}">{value}</div></div>',
        unsafe_allow_html=True
    )

if is_anomaly:
    st.markdown(
        f'<br>Severity &nbsp;<span class="sev-badge {sev_cls}">{sev_text}</span>'
        f'&nbsp;&nbsp;<span style="color:#334155;font-size:0.8rem">'
        f'— Consult a neurologist / radiologist</span>',
        unsafe_allow_html=True
    )
...
img_cols   = st.columns(n_img_cols + 1)
...
if show_heatmap and heatmap:
    with img_cols[1]:
        ...
        st.markdown(
            ...
            '🔴 Red = high activation region</div>', unsafe_allow_html=True
        )
...
with img_cols[-1]:
    ...
    for cls_name, pct in sorted(result['probs'].items(), key=lambda x: x[1], reverse=True):
        ...
        st.markdown(
            ...
            f'<div class="prob-bar-bg"><div class="prob-bar-fill" style="width:{pct:.1f}%;background:{bar_color}"></div></div>'
...
fig.patch.set_facecolor('#080c10'); ax.set_facecolor('#080c10')
